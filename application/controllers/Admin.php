...
if ($para1 == 'do_add') {
    ...
    function category_digital($para1 = '', $para2 = '')
    {
        ...
        if ($para1 == 'do_add') {
            $data['category_id'] = $this->input->post('category_name');//farooq
            $data['category_name'] = $this->input->post('category_name');
            $data['digital'] = 'ok';
            $this->db->insert('category', $data);
            $id = $this->db->insert_id();
    ...
    function sub_category($para1 = '', $para2 = '')
    {
        ...
        if ($para1 == 'do_add') {
            $data['sub_category_name'] = $this->input->post('sub_category_name');
             $array = [" ", "&"];
            $replacewith   = ["", "_"];
            $data['sub_category_id'] = str_replace($array, $replacewith, $this->input->post('sub_category_name'));
            
            // $data['sub_category_id'] = $this->input->post('sub_category_name');
    ...
    function product($para1 = '', $para2 = '', $para3 = '')
    {
        ...
        if ($para1 == 'do_add') {
            ...
            foreach ($products as $row) {
                ...
                $res['image']  = '<img class="img-sm" style="height:auto !important; border:1px solid #ddd;padding:2px; border-radius:2px !important;" src="'.$this->crud_model->file_view('product',$row['product_id'],'','','no','src','multi','one').'"  />';
                $res['title']  = $row['title'];
            ...
            foreach ($products as $row) {
                ...
                $res['options'] = "  <a class=\"btn btn-info btn-xs btn-labeled fa fa-location-arrow\" data-toggle=\"tooltip\" 
                            ...
                            <a class=\"btn btn-info btn-xs btn-labeled fa fa-location-arrow\" data-toggle=\"tooltip\" 
                                onclick=\"ajax_modal('nic_copy','".translate('payment_details')."','".translate('successfully_saved!')."','vendor_nic_copy','".$row['package_payment_id']."');proceed('to_list');\" data-original-title=\"View\" data-container=\"body\">
                                    ".translate('view')."
                            </a>
    ...
    function vendor($para1 = '', $para2 = '', $para3 = '')
    {
        ...
        } else if ($para1 == 'nic_copy') {
            $page_data['vendor_data'] = $this->db->get_where('vendor', array(
                'vendor_id' => $para2
            ))->result_array();
            $this->load->view('back/admin/vendor_nic_copy', $page_data);
        } else if ($para1 == 'view') {
            $page_data['vendor_data'] = $this->db->get_where('vendor', array(
                'vendor_id' => $para2
            ))->result_array();
   ...
   /* Manage Frontend Captcha Settings Credentials */
    ...
    /*code by saqib waheed for data entry*/
    function add_brand_to_all_categories()
    {
//        $subcats = $this->db->get("sub_category")->result_array();
//        $total = count($subcats);
//        $updated = 0;
//        foreach ($subcats as $sc)
//        {
//            $brands = json_decode($sc["brand"]);
//            if(is_array($brands))
//            {
//                array_push($brands,"278");
//                $this->db->where("sub_category_id",$sc["sub_category_id"]);
//                $this->db->update("sub_category",array("brand"=>json_encode($brands)));
//                $updated++;
//            }
//
//        }
//        echo $updated." sub categories updated out of ".$total;
    }
