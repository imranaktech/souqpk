...
public function sendEmail($data){
   ...
   $mesg = $this->load->view('emailtemplates/contact',$data,TRUE);
   ...
   if ($this->email->send()) {
    ...
    function profile($para1 = "", $para2 = "", $para3 = "")
    {
        ...
        if($para1=='dlt_img'){
        $imageName='user_'.$this->session->userdata('user_id').'.jpg';
        if (file_exists("uploads/user_image/".$imageName)) {
                unlink("uploads/user_image/".$imageName);
                echo json_encode(['code' =>200,'msg'=>'Record deleted Successfully']);die;
             }
         }
    ...
    function ticket_message_add()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if ($safe == 'yes') {
                ...
                $ticketData['tomail']='support@souqpk.com';
                $ticketData['subject']=$this->input->post('sub');
                $ticketData['message']=$data1['message'];
                $this->sendEmail($ticketData);
                echo 'success#-#-#';
            } else {
    ...
    function category($para1 = "", $para2 = "", $min = "", $max = "", $text = '')
    {
        ...
        $page_data['page_title'] = translate('products');
        $page_data['all_category'] = $this->db->order_by("category_name", "asc")->get('category')->result_array();
        $page_data['all_sub_category'] = $this->db->get('sub_category')->result_array();
        $page_data['cur_sub_category'] = $sub;
        $page_data['cur_brand'] = $brand;
        $page_data['cur_category'] = $para1;
        ...
        $page_data['category_data'] = $this->db->order_by("category_name", "asc")->get_where('category', array(
            'category_id' => $para1
        ))->result_array();
        $this->load->view('front/index', $page_data);
    }
...
//            $this->form_validation->set_rules('zip', 'Zip', 'required');
            $this->form_validation->set_rules('nic_no', 'NIC Number', 'required');
            if (empty($_FILES['nic_copy']['name']))
            {
                $this->form_validation->set_rules('nic_copy', 'NIC Front and Back side', 'required');
            }
                ...
                if ($safe == 'yes') {
                    if ($this->crud_model->get_settings_value('general_settings', 'captcha_status', 'value') == 'ok') {
                        ...
                        if ($response['success']) {
                            ...
                            $filename=$_FILES["nic_copy"]["name"];
                            $temp_name=$_FILES["nic_copy"]["tmp_name"];
                            $folder="uploads/vendor_nic_image".$filename;
                            move_uploaded_file($temp_name,$folder);
                            $data['nic_copy'] = $folder;
                    // $data['nic_copy'] = $_FILES["uploadfile"]["name"];;
            ...
            if($msg=="done")
            {
                $this->crud_model->file_up('nic_copy',"vendor_nic",$this->db->insert_id('vendor_id'),"","no","");
            }
...
function fblogin(){
    ...
    $fb = new Facebook\Facebook([
          'app_id' => '966893080761291',
          'app_secret' => '9e98c0c37b6b33c0671aaf6d59100837',
...
// For more permissions like user location etc you need to send your application for review
   ...
   $redirectURL = base_url().'home/fbcallback';
...
if (!session_id()) {
    session_start();
        ...
        if ($user_id = $this->crud_model->exists_in_table('user', 'fb_id', $me->getProperty('id'))) {
                } else {
                    $data['username'] = $me->getProperty('first_name');
                    $data['surname'] = $me->getProperty('last_name');
                    $data['email'] = $me->getProperty('email');
                    $data['wishlist'] = '[]';
                    $data['package_info'] = '[]';
                    $data['product_upload'] = $this->db->get_where('package', array('package_id' => 1))->row()->upload_amount;
                    $data['fb_id'] = $me->getProperty('id');

                    $data['g_photo'] = $userProfile['picture'];
    ...
    function googlelogin()
{
    ...
    require_once APPPATH.'third_party/src/contrib/Google_Oauth2Service.php';
        ...
        $clientId = '777545987803-lg452o6d6ihfje2b1u7afrmug8189ms0.apps.googleusercontent.com'; //Google client ID
        $clientSecret = 'dtnPoxwMy_Qw_nvmYlGNNbSA'; //Google client secret
        $redirectURL = base_url().'home/googlelogin';
        ...
        if ($gClient->getAccessToken()) {
            $userProfile = $google_oauthV2->userinfo->get();
                if ($user_id = $this->crud_model->exists_in_table('user', 'g_id', $userProfile['id'])) {
                } else {
                    $data['username'] = $userProfile['given_name'];
                    $data['surname'] = $userProfile['family_name'];
                    $data['email'] = $userProfile['email'];
                    $data['wishlist'] = '[]';
                    $data['package_info'] = '[]';
                    $data['product_upload'] = $this->db->get_where('package', array('package_id' => 1))->row()->upload_amount;
                    $data['g_id'] = $userProfile['id'];

                    $data['g_photo'] = $userProfile['picture'];
                    $data['creation_date'] = time();
                    $data['password'] = substr(hash('imran1122', rand()), 0, 12);
                    $this->db->insert('user', $data);
                    $user_id = $this->db->insert_id();
                }
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if ($signin_data->num_rows() > 0) {
                    foreach ($signin_data->result_array() as $row) {
                        ...
                        if(!empty($remember))
                        {
                            $_SESSION["souq_user_email"]=$this->input->post('email');
                            $_SESSION["souq_user_password"]=$this->input->post('password');
                        }else{
                            if(!empty($_SESSION["souq_user_email"]))
                            {
    ...
    function login_set($para1 = '', $para2 = '', $para3 = '')
    {
        ...
        if ($g_login_set == 'ok') {
            ...
            if (@$_SESSION['token']) {
                ...
                // $this->googleplus->client->setAccessToken($_SESSION['access_token']);farooq
            }
        ...
        if ($para1 == 'login') {
            ...
            $page_data['email'] = !empty($_SESSION['souq_user_email'])?$_SESSION['souq_user_email']:'';
            $page_data['password'] = !empty($_SESSION['souq_user_password'])?$_SESSION['souq_user_password']:'';
    ...
    function cart_finish($para1 = "", $para2 = "")
    {
        ...
        if ($this->input->post('payment_type') == 'paypal') {
            ...
            if ($para1 == 'go') {
                ...
                $data['payment_status'] = '[]';
                $data['c_o_d_status'] = '[]'; //update by farooq12/1/2020
                $data['order_status'] = '[]'; //update by farooq12/2/2020
                $data['payment_details'] = '';
                $data['grand_total'] = $grand_total;
                $data['sale_datetime'] = time();
                $data['delivary_datetime'] = '';
                ...
                $delivery_status = array();
                $payment_status = array();
                $c_o_d_status = array();//update by farooq12/1/2020
                $order_status = array();//update by farooq12/2/2020
                foreach ($vendors as $p) {
                    $delivery_status[] = array('vendor' => $p, 'status' => 'pending', 'comment' => '', 'delivery_time' => '');
                    $payment_status[] = array('vendor' => $p, 'status' => 'due');
                    $c_o_d_status[] = array('vendor' => $p, 'status' => 'due');//update by farooq12/1/2020
                    $order_status[] = array('vendor' => $p, 'status' => 'pending','reminder'=>date('Y-m-d H:i:s', strtotime('+29 hours', time())));//update by farooq12/2/2020
                }
                if ($this->crud_model->is_admin_in_sale($sale_id)) {
                    $delivery_status[] = array('admin' => '', 'status' => 'pending', 'comment' => '', 'delivery_time' => '');
                    $payment_status[] = array('admin' => '', 'status' => 'due');
                    $c_o_d_status[] = array('admin' => '', 'status' => 'due');//update by farooq12/1/2020
                    $order_status[] = array('admin' => '', 'status' => 'pending');//update by farooq12/2/2020
                }
                $data['sale_code'] = date('Ym', $data['sale_datetime']) . $sale_id;
                $data['delivery_status'] = json_encode($delivery_status);
                $data['payment_status'] = json_encode($payment_status);
                $data['c_o_d_status'] = json_encode($c_o_d_status); //update by farooq12/1/2020
                $data['order_status'] = json_encode($order_status); //update by farooq12/1/2020
                $this->db->where('sale_id', $sale_id);
                $this->db->update('sale', $data);
    ...
    function others_product($para1 = "")
    {
        ...
        function todays_deal($para1 = "")
    {
        $page_data['product_type'] = 'todays_deal';
        $page_data['page_name'] = 'others_list';
        $page_data['asset_page'] = 'product_list_other';
        $page_data['page_title'] = translate($para1);
        $this->load->view('front/index', $page_data);
    ...
    {
        $page_data['product_type'] = 'featured';
        $page_data['page_name'] = 'others_list';
        $page_data['asset_page'] = 'product_list_other';
        $page_data['page_title'] = translate($para1);
        $this->load->view('front/index', $page_data);
    ...
    /*pages for android*/
    /* FUNCTION: Loads Custom Pages */
    function android_page($parmalink = '')
    {
        $pagef = $this->db->get_where('page', array(
            'parmalink' => $parmalink
        ));
        ...
        $this->load->view('front/android_page', $page_data);
    }
