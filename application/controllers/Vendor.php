...
if ($para1 == 'do_add') {
    ...
    $data['paltform']         = $this->input->post('paltform');
    $data['vender_name']         = $this->input->post('vender_name');
    $data['price']         = $this->input->post('price');
    $data['vender_phone']         = $this->input->post('vender_phone');
    $data['unique_code']         = mt_rand(100000, 999999);

    $this->db->insert('paid_services', $data);
    $data['type']='payment_proof';
    $data['tomail']='info@souqpk.com';
    $data['name']=$data['vender_name'];
    $data['subject']='Payment Proof';
    $id = $this->db->insert_id();
    sendEmail($data);
    if(!demo()){
        $this->crud_model->file_up("img", "paid_service", $id, '', '', '.jpg');
    }
    $this->session->set_flashdata('success',translate('Record Added Successfully'));
    redirect('vendor/paid_services/add_service');
    
} else if ($para1 == 'add_service') {
    $page_data['page_name']      = "paid_service_add";
    $this->load->view('back/index', $page_data);
} elseif ($para1 == "update") {
    ...
    $page_data['data'] = $this->db->get('paid_services')->result_array();
    $this->load->view('back/vendor/paid_services_list', $page_data);
} elseif ($para1 == 'add') {
    $this->load->view('back/vendor/paid_service_add');
    ...
    $page_data['page_name']      = "paid_service";
    ...
    {
        if(!demo()){
            $this->crud_model->imageCompressor();
        }
    }
   ...
   function product($para1 = '', $para2 = '', $para3 = '')
    {
        ...
        if($para1 == 'change_status'){
            $productids = $this->input->post('product_ids');
            $type=$this->input->post('typeValue');

            if($type=='delete'){
                $this->db->where_in('product_id', $productids);
                $this->db->delete('product');
                echo json_encode(['code' =>200,'msg'=>'Record '.$type.' Successfully']);die;
            }
        ...
        if ($para1 == 'do_add') {
            ...
            $enableSize=$this->input->post('enableSize');
            if($enableSize==1){
                $data['enable_size']  = $enableSize;
                $data['sizes']  = implode(',',$this->input->post('sizes'));
            }else{
                $data['enable_size']  = 0;
            }
            ...
            $data['deal']               = $this->input->post('today_deal');
            ...
            $data['added_by']           = json_encode(array('type'=>'vendor','id'=>$this->session->userdata('vendor_id')));
            $productImages              = json_decode($this->input->post('Images'));
            ...
            foreach ($products as $row) {
                ...
                if($row['product_id']>1043){
                    $res['image']  = '<img class="img-sm" style="height:auto !important; border:1px solid #ddd;padding:2px; border-radius:2px !important;" src="'.$this->crud_model->getImage('product_images','single',$row['product_id'],'image_name','product_id','product').'"  />';
                }else{
            ...
            if(!demo()){
                ...
                $this->db->where('image_name', $para2);
            ...
            $brands=json_decode($this->crud_model->get_type_name_by_id('sub_category',$para2,'name'),true);
       ...
       $inputFileType = $this->spreadsheet->identify($inputFileName);
        ...
        if(!empty($products)){
            foreach ($products as $product){
                ...
                echo json_encode(['file' => $fileName,'code'=>200]);die;


        // $this->session->set_flashdata('success',translate('Products uploaded '.$fileName));
        // redirect('vendor/product_bulk_upload');
    ...
    public function product_bulk_upload_save_single($product)
    {
        ...
        $product_data['category'] = $product['category'] ? $product['category'] : '';
        $product_data['sub_category'] = $product['sub_category'] ? $product['sub_category'] : '';
    ...
    function sales($para1 = '', $para2 = '')
    {
        ...
        if ($para1 == 'delete') {
        ...
        } elseif ($para1 == 'delivery_payment_cod') {
            $data['viewed'] = 'ok';
            $this->db->where('sale_id', $para2);
            $this->db->update('sale', $data);
            ...
            $this->load->view('back/vendor/Payment_status', $page_data);
        }  elseif ($para1 == 'delivery_payment_set_cod') {
            ...
            $data['c_o_d_status'] = json_encode($new_delivery_status);
            $data['payment_details'] = $this->input->post('payment_details');
            $this->db->where('sale_id', $para2);
            $this->db->update('sale', $data);
        }elseif ($para1 == 'order_status') {
            $data['viewed'] = 'ok';
            $this->db->where('sale_id', $para2);
            $this->db->update('sale', $data);
            $page_data['sale_id']         = $para2;
            ...
            foreach ($payment_status as $row) {
                if(isset($row['vendor'])){
                    if($row['vendor'] == $this->session->userdata('vendor_id')){
                        $page_data['order_status'] = $row['status'];
                    }
                }
            }
            
            $this->load->view('back/vendor/order_status', $page_data);
        }elseif ($para1 == 'order_payment_set') {
            $delivery_status = json_decode($this->db->get_where('sale', array(
                'sale_id' => $para2
            ))->row()->delivery_status,true);
            $new_delivery_status = array();
            ...
            var_dump($new_payment_status);
            // $data['payment_status']  = json_encode($new_payment_status);
            $data['order_status'] = json_encode($new_delivery_status);
            $data['payment_details'] = $this->input->post('payment_details');
            $this->db->where('sale_id', $para2);
            $this->db->update('sale', $data);
            ...
            foreach ($delivery_status as $row) {
                if(isset($row['vendor'])){
                    if($row['vendor'] == $this->session->userdata('vendor_id')){
                        $date=date('Y-m-d H', strtotime('+77 hours', time()));
                        $new_delivery_status[] = array('vendor'=>$row['vendor'],'status'=>$this->input->post('delivery_status'),'comment'=>$this->input->post('comment'),'delivery_time'=>time(),'reminder_data'=>$date);
                    } else {
                        $new_delivery_status[] = array('vendor'=>$row['vendor'],'status'=>$row['status'],'comment'=>$row['comment'],'delivery_time'=>$row['delivery_time']);
                    }
                }
      ...
      function manage_vendor($para1 = "")
    {
        ...
        if ($para1 == 'update_password') {
          ...
          if(isset($_FILES['profile_image']) && !empty($_FILES['profile_image']['name'])) {
            $ImageName = time().$_FILES['profile_image']['name'];
            $path = 'vender/'; 
    ...
    if(!demo()){
        ...
        $id=uniqid();
         if(isset($_FILES['logo']) && !empty($_FILES['logo']['name'])) {
             $path = $_FILES['logo']['name'];
             $ext = pathinfo($path, PATHINFO_EXTENSION);
             $this->crud_model->file_up("logo", "vendor_logo", $id, '','no','.'.$ext);
             $logo="vendor_logo_".$id.'.'.$ext;;
        }else{
            $logo=$this->input->post('preLogo');
        }

         if(isset($_FILES['banner']) && !empty($_FILES['banner']['name'])) {
             $path = $_FILES['banner']['name'];
             $ext = pathinfo($path, PATHINFO_EXTENSION);
            $this->crud_model->file_up("banner", "vendor_banner", $id, '','no','.'.$ext);
            $banner="vendor_banner_".$id.'.'.$ext;
        }else{
            $banner=$this->input->post('prebanner');
        }
