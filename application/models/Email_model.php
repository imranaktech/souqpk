...
class Email_model extends CI_Model
{
    ...
    function email_invoice($sale_id){
        ...
        // Send Email to Venders
        $saleData= $this->db->get_where('sale', array('sale_id' => $sale_id))->row()->order_status;
        foreach (json_decode($saleData) as  $value) {

        $venderData=$this->venderDetail($value->vendor);
        $emailData=$this->venderProductDetailOrder($sale_id,$value->vendor);
        $venderView  = $this->load->view('front/shopping_cart/vender_email', $emailData, TRUE);
        $this->do_email($from, $from_name, $venderData->email, $subject, $venderView);
        
        }


        $admins     = $this->db->get_where('admin',array('role'=>'5'))->result_array();
...
function venderProductDetailOrder($saleId,$venderId)
    {
