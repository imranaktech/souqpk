...
class Crud_model extends CI_Model
{
    ...
    function filter_one($table, $type, $value)
    {
     ...
     public  function imageCompressor()
     {
        ...
        else
        {
            ...
            for ($i=0; $i <$count ; $i++) { 
                $data = uniqid().'_'.$_FILES['images']['name'][$i];
                $config['image_library'] = 'gd2';
                $config['source_image'] = $_FILES['images']['tmp_name'][$i];
                $config['remove_spaces'] = true;
                $config['create_thumb'] = false;
                $config['maintain_ratio'] = false;
                $config['width'] = 650;
                $config['height'] = 650;
                $config['new_image'] = 'uploads/product_image/' . $data;
            ...
            echo json_encode(['files' => $filesArray]);die;
     ...
     function getImage($table,$type,$condition,$select,$where,$folder)
    {
        ...
        if($type=='single'){
            ...
            if($data){
              if (file_exists('uploads/' . $folder . '_image/'.$data->image_name)) {
                return base_url() . 'uploads/' . $folder . '_image/'.$data->image_name;
            }  
            }
            else{
                return base_url() . 'uploads/' . $folder . '_image/default.jpg';
            }
    ...
    // FILE_UPLOAD
    function img_thumb($type, $id, $ext = '.jpg', $width = '650', $height = '650')
    {
        ...
        // else
        // {
        //     echo 'hii'.$this->image_lib->display_errors();
        // }
   ...
   function file_up_from_urls($urls, $type, $id, $no_thumb = '')
{
    ...
    /////////GET CHOICE TITLE////////
    function choice_title_by_name($product_id, $name)
    {
        $return = '';
    ...
    function email_invoice($sale_id)
    {
        ...
        $text = $this->load->view('front/shopping_cart/invoice2_email', $page_data, TRUE);
    ...
    function vendor_share_total($vendor_id, $pay = '', $pay_type = '')
    {
        ...
        $shipping = 0;
        $total = 0;
        $commission = $this->db->get_where('business_settings', array('type' => 'commission_amount'))->row()->value;
        $sales = $this->db->get('sale')->result_array();
        foreach ($sales as $row) {
        ...
        return array('price' => $product_price, 'tax' => $tax, 'shipping' => $shipping, 'total' => $total, 'total_commission' => $total_commission);
    }
    
    function venderTotalSale($vendor_id)
    {
        $stock = 0;
        ...
        foreach ($data as  $value) {
            echo "<pre>";
            foreach (json_decode($value->c_o_d_status) as $value1) {
                if($value1->vendor==$vendor_id){
                    $stock += $value->grand_total;
                }
            }
        }
    ...
    function rating_count($product_id)
    {
        $num = $this->get_type_name_by_id('product', $product_id, 'rating_num');
        return $num;
    }
    ...
    function vendor_rating_count($id)
    {
        $this->db->where('added_by', json_encode(array('type' => 'vendor', 'id' => $id)));
        $products = $this->db->get('product')->result_array();
        $num = 0;
    ...
    function seo_stat($type = '')
    {
        try {
            ...
            if ($seostats->setUrl($url)) {
                ...
                if ($type == 'facebook') {
                    return SEOstats\Services\Social::getFacebookShares($url);
                } elseif ($type == 'gplus') {
                    return SEOstats\Services\Social::getGooglePlusShares($url);
                } elseif ($type == 'twitter') {
                    return SEOstats\Services\Social::getTwitterShares($url);
                } elseif ($type == 'linkedin') {
                    return SEOstats\Services\Social::getLinkedInShares($url);
                } elseif ($type == 'pinterest') {
                    return SEOstats\Services\Social::getPinterestShares($url);
                }elseif ($type == 'alexa_page_load_time') {
                    return SEOstats\Services\Alexa::getPageLoadTime($url);
                } elseif ($type == 'alexa_global') {
                    return SEOstats\Services\Alexa::getGlobalRank($url);
                } elseif ($type == 'alexa_country') {
                    return SEOstats\Services\Alexa::getCountryRank($url);
                } elseif ($type == 'alexa_bounce') {
                    return SEOstats\Services\Alexa::getTrafficGraph(5,$url,400,200);
                } elseif ($type == 'alexa_time') {
                    return SEOstats\Services\Alexa::getTrafficGraph(4,$url,400,200);
                }elseif ($type == 'alexa_search_visits') {
                    return SEOstats\Services\Alexa::getTrafficGraph(6,$url,400,200);
                } elseif ($type == 'alexa_traffic') {
                    return SEOstats\Services\Alexa::getTrafficGraph(1,$url,400,200);
                } elseif ($type == 'alexa_pageviews') {
                    return SEOstats\Services\Alexa::getTrafficGraph(2,$url,400,200);
                } elseif ($type == 'google_siteindex') {
                    return SEOstats\Services\Google::getSiteindexTotal($url);
                } elseif ($type == 'google_back') {
                    return SEOstats\Services\Google::getBacklinksTotal($url);
                } elseif ($type == 'search_graph_1') {
                    return SEOstats\Services\SemRush::getDomainGraph(1,$url,false,400,200);
                } elseif ($type == 'search_graph_2') {
                    return SEOstats\Services\SemRush::getDomainGraph(2,$url,false,400,200);
                }
    ...
    function compared_num()
    {
        ...
        if (!empty($this->session->userdata('compare'))) {
            if ($this->is_json($this->session->userdata('compare'))) {
                $decoded_compare = json_decode($this->session->userdata('compare'), true);
                if (is_array($decoded_compare)) {
                    $count = count($decoded_compare);
                }
            }
        }
        return $count;
...
>
    ...
    */
    ...
    function is_product_bought($buyer_id, $product_id)
    {
        ...
        $this->db->select('*');
        $this->db->from('sale');
        $this->db->where('buyer', $buyer_id);
        $this->db->like('product_details', '"id":"' . $product_id . '"');
        $this->db->like('payment_status', '"status":"paid"');
        $this->db->limit(1);
    ...
    function is_product_affiliation_on($product_id)
    {
        $bool = 0;

        $this->db->select('product.product_id,sub_category.sub_category_id,sub_category.affiliation');
    ...
    function get_product_affiliation_from_subcategory($product_id)
    {
        $this->db->select('product.product_id,sub_category.sub_category_id,sub_category.affiliation,sub_category.affiliation_points');
        $this->db->from('product');
        $this->db->join('sub_category', 'product.sub_category=sub_category.sub_category_id');
        $this->db->where('product.product_id', $product_id);
        $this->db->limit(1);
    ...
    function get_affiliation_link($product_id, $affiliator_id)
    {
        $link = "";
        if (is_numeric($affiliator_id)) {
            $affiliation = $this->get_product_affiliation($product_id, $affiliator_id);

            if (!empty($affiliation)) {
                $link = $this->product_link($product_id) . "/?product_affiliation_code={$affiliation['product_affiliation_code']}";
            }
        }
    ...
    function get_or_create_affiliation_link($product_id, $affiliator_id)
    {
        $link = "";
        if (is_numeric($affiliator_id)) {
            $affiliation = $this->get_or_insert_product_affiliation($product_id, $affiliator_id);
            $link = $this->product_link($product_id) . "/?product_affiliation_code={$affiliation['product_affiliation_code']}";
        }
        return $link;
    }

    function get_or_insert_product_affiliation($product_id, $affiliator_id)
    {
        $affiliation = $this->get_product_affiliation($product_id, $affiliator_id);

        if (empty($affiliation)) {
            $this->insert_product_affiliation($product_id, $affiliator_id);
            $affiliation = $this->get_product_affiliation($product_id, $affiliator_id);
        }
    ...
    function get_product_affiliation($product_id, $affiliator_id)
    {
        return $this->db->get_where('product_affiliation', array('product_id' => $product_id, 'product_affiliator_id' => $affiliator_id))->row_array();
    }

    function get_product_affiliation_by_code($product_affiliation_code)
    {
        return $this->db->get_where('product_affiliation', array('product_affiliation_code' => $product_affiliation_code))->row_array();
    }

    function get_product_affiliator_id_by_code($product_affiliation_code)
    {
        $affiliator_id = 0;
        $affiliation = $this->get_product_affiliation_by_code($product_affiliation_code);
        if (!empty($affiliation)) {
            $affiliator_id = $affiliation['product_affiliator_id'];
        }
        return $affiliator_id;
    }


    function insert_product_affiliation($product_id, $affiliator_id)
    {
        $product_affiliation_code = $this->_generate_key('product_affiliation', 'product_affiliation_code', 'aff');
        $affiliation = array(
            'product_id' => $product_id,
            'product_affiliator_id' => $affiliator_id,
            'product_affiliation_code' => $product_affiliation_code
        );

        $this->db->set($affiliation);
        $this->db->insert('product_affiliation');
    }

    function update_product_affiliation($product_id, $affiliator_id)
    {
        $product_affiliation_code = $this->_generate_key('product_affiliation', 'product_affiliation_code', 'aff');
        $affiliation = array(
            'product_affiliation_code' => $product_affiliation_code
        ...
        $this->db->update('product_affiliation', $affiliation, $condition);
    }

    function get_product_affiliation_codes_from_cookies()
    {
        $product_affiliation_codes_string = $this->input->cookie('product_affiliation_codes', true);
        $codes = array_filter(explode(",", $product_affiliation_codes_string)); //filtering empty values

        $product_affiliation_codes =
            !empty($codes) && is_array($codes)
                ? $codes : array();

        return $product_affiliation_codes;
    }

    function set_product_affiliation_code_in_cookies($product_affiliation_code)
    {
        $product_affiliation_codes = $this->get_product_affiliation_codes_from_cookies();

        //duplicate check
        if (!in_array($product_affiliation_code, $product_affiliation_codes)) {
            $product_affiliation_codes[] = $product_affiliation_code;
            $product_affiliation_codes_string = implode(",", $product_affiliation_codes);
            $this->set_cookie('product_affiliation_codes', $product_affiliation_codes_string, 90);
        }
        return $product_affiliation_codes_string;
    }

    function remove_used_codes_from_cookies($used_codes)
    {
        if (!empty($used_codes) && is_array($used_codes)) {
            $cookie_codes = $this->get_product_affiliation_codes_from_cookies();
            $new_codes = array_values(array_diff($cookie_codes, $used_codes));

            //delete and reset
            //$this->delete_cookie('product_affiliation_codes');
            $product_affiliation_codes_string = !empty($new_codes) ? implode(",", $new_codes):'';
            $this->set_cookie('product_affiliation_codes', $product_affiliation_codes_string, 90);

            /*echo "<br>code diffs<br>";
            echo "<pre>";
            print_r($cookie_codes);
            print_r($used_codes);
            print_r($new_codes);
            echo "product_affiliation_codes_string:$product_affiliation_codes_string";
            print_r($this->get_product_affiliation_codes_from_cookies());
            echo "</pre>";*/
        }
    ...
    function get_affiliations_from_affiliation_codes_from_cookies()
    {

        $product_affiliation_codes = $this->get_product_affiliation_codes_from_cookies();
        $affiliations = array();
        if (!empty($product_affiliation_codes)) {
            $affiliations = $this->db->select('*')->from('product_affiliation')->where_in('product_affiliation_code', $product_affiliation_codes)->get()->result_array();
        }
    ...
    function process_affiliation($sale_id,$check_if_paid = false)
    {
        //process only if logged in
        $used_codes = array();
        if ($this->session->userdata('user_login') == "yes" && $this->crud_model->get_settings_value('general_settings', 'product_affiliation_set', 'value') == 'ok') {
            ...
            if (!empty($sale)) {
                ...
                $payment_status = json_decode($sale['payment_status'], true);

                /*echo "<br>pd and ps<br>";
                echo "<pre>";
                print_r($product_details);
                print_r($payment_status);
                echo "<br> paid? {$payment_status[0]['status']}";
                echo "</pre>";*/

                //if given paid check is false automatically considered as paid else check the actual result
                $paid = $check_if_paid == false ? true :  $payment_status[0]['status'] == 'paid';

                $product_ids_in_sale = array();
                if ($paid) {
                    foreach ($product_details as $product_detail) {
                        $product_ids_in_sale[] = $product_detail['id'];

                        //echo "<br> pid push {$product_detail['id']} <br>";
                    }
                }

                /*echo "<br>pids in sale<br>";
                echo "<pre>";
                print_r($product_ids_in_sale);
                echo "</pre>";*/

                if (!empty($product_ids_in_sale)) {
                    $affiliations = $this->get_affiliations_from_affiliation_codes_from_cookies();

                    /*echo "<br>affiliations<br>";
                    echo "<pre>";
                    ...
                    if (!empty($affiliations)) {
                        foreach ($affiliations as $affiliation) {
                            if (in_array($affiliation['product_id'], $product_ids_in_sale)
                                && $affiliation['product_affiliator_id'] != $user_id
                                && !$this->is_affiliation_code_used_by_user($affiliation['product_affiliation_code'],$user_id)
                            ) {
                                //echo "vars: <br>{$affiliation['product_id']}, {$affiliation['product_affiliator_id']}, $user_id, {$affiliation['product_affiliation_code']}<br>";

                                $this->single_affiliation_code_use_process($affiliation['product_id'], $affiliation['product_affiliator_id'], $user_id, $affiliation['product_affiliation_code']);
                                $used_codes[] = $affiliation['product_affiliation_code'];
                            }
                        }
                    }
                }
        ...
        if (!empty($used_codes)) {
            $this->remove_used_codes_from_cookies($used_codes);
        }
    }

    function single_affiliation_code_use_process($product_id, $affiliator_id, $affiliation_user_id, $code)
    {
        $product_affiliation_from_subcategory = $this->get_product_affiliation_from_subcategory($product_id);

        if (!empty($product_affiliation_from_subcategory)) {
            if ($product_affiliation_from_subcategory['affiliation'] == 1
                && is_numeric($product_affiliation_from_subcategory['affiliation_points'])
                && $product_affiliation_from_subcategory['affiliation_points'] > 0) {
                $equivalent_currency = $this->get_equivalent_currency_for_affiliation_points($product_affiliation_from_subcategory['affiliation_points']);
                $this->set_product_affiliation_points_total($product_affiliation_from_subcategory['affiliation_points'],$equivalent_currency, $affiliator_id);
                $this->insert_product_affiliation_code_use($product_affiliation_from_subcategory['affiliation_points'],$equivalent_currency, $affiliator_id, $affiliation_user_id, $code, $product_id);
                $this->convert_affiliation_points_to_wallet($equivalent_currency,$affiliator_id);
            }
        }
    }

    function convert_affiliation_points_to_wallet($equivalent_currency,$affiliator_id){
        $this->wallet_model->add_user_balance($equivalent_currency,$affiliator_id);

        $ins_wallet_load = array();
        $ins_wallet_load['user'] = $affiliator_id;
        $ins_wallet_load['method'] = 'affiliation_point_conversion';
        $ins_wallet_load['amount'] = $equivalent_currency;
        $ins_wallet_load['status'] = 'paid';
        $ins_wallet_load['payment_details'] = '[]';
        $ins_wallet_load['timestamp'] = time();
        $this->db->insert('wallet_load', $ins_wallet_load);
    }

    function set_product_affiliation_points_total($points,$equivalent_currency, $affiliator_id)
    {
        $product_affiliation_earning_total = $this->get_product_affiliation_earning_total($affiliator_id);

        //insert or update
        if (empty($product_affiliation_earning_total)) {
            $ins = array(
                'affiliator_id' => $affiliator_id,
                'points' => $points,
                'currency' => $equivalent_currency,
                'updated_at' => date("Y-m-d H:i:s")
            );

            $this->db->set($ins);
            $this->db->insert('product_affiliation_points_total');
        } else {

            $upd = array(
                'points' => $product_affiliation_earning_total['points'] + $points,
                'currency' => $product_affiliation_earning_total['currency'] + $equivalent_currency,
                'updated_at' => date("Y-m-d H:i:s")
            );
            ...
            $this->db->update('product_affiliation_points_total', $upd, $condition);
        }

    }

    function get_equivalent_currency_for_affiliation_points($points)
    {
        $affiliation_point_to_currency_rate = $this->get_settings_value('general_settings', 'affiliation_point_to_currency_rate', 'value');
        return is_numeric($affiliation_point_to_currency_rate) && $affiliation_point_to_currency_rate > 0 ? $affiliation_point_to_currency_rate * $points : 1.00;

    }

    function get_product_affiliation_earning_total($affiliator_id)
    {
        return $this->db->select('*')->from('product_affiliation_points_total')->where('affiliator_id', $affiliator_id)->get()->row_array();
    }

    function insert_product_affiliation_code_use($affiliation_points,$equivalent_currency, $affiliator_id, $affiliation_user_id, $code, $product_id)
    {
        $ins = array(
            'affiliator_id' => $affiliator_id,
            'code' => $code,
            'affiliation_user_id' => $affiliation_user_id,
            'points' => $affiliation_points,
            'currency' => $equivalent_currency,
            'product_id' => $product_id,
        ...
        $this->db->set($ins);
        $this->db->insert('product_affiliation_code_use');
    }

    function get_product_affiliation_code_used($code)
    {
        return $this->db->select('*')->from('product_affiliation_code_use')->where('code', $code)->get()->row_array();
    }

    function is_affiliation_code_used($code)
    {
        return $this->db->select('*')->from('product_affiliation_code_use')->where('code', $code)->limit(1)->get()->num_rows() > 0 ? true : false;
    }

    function is_affiliation_code_used_by_user($code,$affiliation_user_id)
    {
