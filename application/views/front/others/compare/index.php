            ...
            <table class="table-bordered table-hover" data-cat="<?php echo $row['category']; ?>">
                <thead>
                    <tr class="product cat_<?php echo $row['category']; ?>" data-cat="<?php echo $row['category']; ?>">
                        <th style="font-size: 13pt;"><?php echo translate('Product');?></th>
                        <?php
                            $i = 0;
                        ...
                        <td class="colm_<?php echo $row['category']; ?>_<?php echo $i; ?>_name col-md-3" >
                            <label style="width:200px;">
                                <select data-placeholder="<?php echo translate('choose_a_product'); ?>" class="searcher selectpicker col-md-12" data-col="<?php echo $i; ?>"  data-live-search="true" tabindex="2">
                                    ...
                                    ?>
                                    <option style="width:200px;" value="<?php echo $ty['product_id']; ?>" <?php if($ty['product_id'] == $p){ echo 'selected'; } ?> ><?php echo $ty['title']; ?></option>
                                    <?php
                                        }
                                    ?>
                                </select>
                ...
                <tbody>
                    <tr class="cat_<?php echo $row['category']; ?>">
                        <th style="font-size: 13pt;"><?php echo translate('image');?></th>
                        <?php
                            $i = 0;
                            $products = $row['products'];
                            foreach ($products as $p) {
                        ...
                        <td class="colm_<?php echo $row['category']; ?>_<?php echo $i; ?>_image col-md-3" >
                            <img src="<?php echo $this->crud_model->file_view('product',$p,'','','no','src','multi','one'); ?>" width="100" />
                        </td>    
                        <?php
                            }
                            while($i < 3){
                    ...
                    <tr class="cat_<?php echo $row['category']; ?>">
                        <th style="font-size: 13pt;"><?php echo translate('price');?></th>
                        <?php
                            $i = 0;
                            $products = $row['products'];
                            foreach ($products as $p) {
                        ...
                        <td class="colm_<?php echo $row['category']; ?>_<?php echo $i; ?>_price col-md-3" >
                            <?php
                            $pr = $this->db->get_where('product',array('product_id'=>$p))->row();
                            $sale_price=!empty($pr->sale_price)?$pr->sale_price:0;
                            $discount =!empty($pr->discount)?$pr->discount:0;
                            $save = $discount/($sale_price/100);
                            echo "sale price : PKR ".$sale_price."<br>";
                            if($discount>0)
                            {
                                echo "discount : PKR ".$discount."<br>";
                                echo "Save : ".$save."%<br>";
                            }
                            ?>
<!--                            --><?php //echo currency($this->db->get_where('product',array('product_id'=>$p))->row()->sale_price); ?>
                    ...
                    <tr class="cat_<?php echo $row['category']; ?>">
                        <th style="font-size: 13pt;"><?php echo translate('brand');?></th>
                        <?php
                            $i = 0;
                            $products = $row['products'];
                            foreach ($products as $p) {
                    ...
                    <tr class="cat_<?php echo $row['category']; ?>">
                        <th style="font-size: 13pt;"><?php echo translate('description');?></th>
                        <?php
                            $i = 0;
                            $products = $row['products'];
                            foreach ($products as $p) {
                    ...
                    <tr class="product cat_<?php echo $row['category']; ?>" data-cat="<?php echo $row['category']; ?>">
                        <th style="font-size: 13pt;"><?php echo translate('Add to Cart');?></th>
                        <?php
                            $i = 0;
                            $products = $row['products'];
                            foreach ($products as $p) {
                        ...
                        ?>
                        <td class="colm_<?php echo $row['category']; ?>_<?php echo $i; ?>_name col-md-3" >
                            <span class="icon-view right " onclick="to_cart(<?php echo $p; ?>,event)" data-toggle="tooltip"
                                  data-original-title="<?php if($this->crud_model->is_added_to_cart($p)){ echo translate('added_to_cart'); } else { echo translate('add_to_cart'); } ?>">
